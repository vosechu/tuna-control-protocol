...
func test_visited_rack_scores_zero_ferret_picks_other():
	# Ferret at (0, 3000). rack_a at (0, 0), rack_b at (0, 1000).
	# Both within perception radius (8 RU = 19200 pu). After visiting rack_a,
	# the ferret must target rack_b instead.
	var ferret_id: int = _make_ferret(0, 3000)
	var rack_a: int = _make_rack(0, 0)
	var rack_b: int = _make_rack(0, 1000)
	# Pre-visit rack_a
