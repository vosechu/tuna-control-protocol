...
# AI-DEV: The happy-path dispense test was deleted — integration
# test_button_press_creates_sealed_can (tests/integration/test_food_loop.gd)
# already asserts `INVALID_ID` ne + `tuna_can` component + `sealed` state,
# which made a surgical mutation on the unit test impossible without
# cascading failures. The remaining unit tests each target a distinct
# branch gate (no-HUM, drain, rack mismatch, radius, arm-HUM, cleanup).
...
# AI-DEV: The press-drains-HUM unit test was deleted — integrations
# test_connected_cable_lets_press_drain_hum (tests/integration/test_cable_drain_loop.gd)
# and test_dispense_drains_and_open_drains_hum (tests/integration/test_food_loop.gd)
# both assert the same reserve-decreases-after-press invariant, which made a
# surgical unit mutation cascade into integration failures.
...
# AI-DEV: The eaten-can-despawns unit test was deleted — integration
# test_full_dispense_open_eat_cleanup_cycle (tests/integration/test_food_loop.gd)
# asserts the same 100-tick cleanup invariant via the full dispense→open→eat
# path, making a surgical unit mutation impossible without integration fallout.
