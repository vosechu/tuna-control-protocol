...
func test_fresh_connect_writes_cable_and_emits() -> void:
	...
	# Happy-path unit proof. Surgical within-suite mutation: skip the
	# `_emit_connect(hum_id, device_id)` call at the tail of handle_connect.
	# Other wiring unit tests assert deny events or ok=true without checking
	# connect-event counts, so they stay green.
